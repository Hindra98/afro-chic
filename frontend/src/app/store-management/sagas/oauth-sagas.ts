...
import {
  ...
  VerifyPinCodeAction,
  VerifyPinCodeFailurePayload,
} from "../actions/oauth";
import { ActionTypes } from "../actions/constants/action-types";
import {
  authenticateUserFailure,
  authenticateUserSuccess,
  authenticateUserSuccessTemp,
  registerFailure,
  registerSuccess,
  signOutFailure,
  signOutSuccess,
  verifyPinCodeFailure,
...
let store;
export const injectStoreInOauthSaga = (_store) => {
  store = _store;
};
...
const callApiToVerifyPinCode = async (command: VerifyIdentityCommand) =>
    ...
    if (response) {
      if (response.hasSucceeded) {
        ...
        if (response.payload.isTwoFactorAuthenticationEnabled) {
          yield put(
            authenticateUserSuccessTemp(
              response.payload as AuthenticateUserSuccessPayload
            )
          );
        } else {
          ...
          setStorage(
            AuthenticationConstants.TENANT_ID,
            Jwt.getClaim(token, "tenantid")
          );
          setStorage(
            AuthenticationConstants.USER_LANGUAGE,
            Jwt.getClaim(token, "userlanguage")
          );
          setStorage(SharedConstants.TOGGLE_SIDEBAR, "true");

          const stateAuth = store.getState(
            (state) => state.authenticatedUserResult
          );
          const subscriptionKey: string =
            stateAuth.authenticatedUser.subscriptionKey;
          if (subscriptionKey && subscriptionKey.length > 0)
            setStorage("subscription_key", subscriptionKey);
          yield put(
            authenticateUserSuccess(
              response.payload as AuthenticateUserSuccessPayload
            )
  ...
  try {
    ...
    if (response) {
      if (response.hasSucceeded) {
        ...
        if (
          getStorage<string>("subscription_key") &&
          getStorage<string>("subscription_key").length > 0
        ) {
          setStorage("is_verified", "true");
        ...
        yield put(
          verifyPinCodeSuccess(response.payload as VerifyIdentitySuccessPayload)
        );
        yield put(
          authenticateUserSuccess(auth as AuthenticateUserSuccessPayload)
        );
      } else {
        let messages: string[] = [];
        response.errorMessages.map((item) => {
        ...
        yield put(
          verifyPinCodeFailure({
            errors: messages,
          } as VerifyPinCodeFailurePayload)
        );
      }
    } else {
      let messages: string[] = [];
    ...
    messages.push(e.message);
    yield put(
      verifyPinCodeFailure({ errors: messages } as VerifyPinCodeFailurePayload)
    );
  }
}
...
function* registerSaga(action: RegisterAction) {
  try {
    const response = yield call(callApiToRegister, action.payload.command);
    if (response) {
      console.log("Response register saga", response);
      setStorage(AuthenticationConstants.USER, response);
      yield put(registerSuccess(response.payload as User));
    } else {
      const messages: string[] = [];
      messages.push("Erreur lors de la connexion au serveur");
      yield put(
        registerFailure({ errors: messages } as RegisterFailurePayload)
      );
    }
  } catch (e) {
    const messages: string[] = [];
    messages.push(e.message);
    yield put(registerFailure({ errors: messages } as RegisterFailurePayload));
  }
}
